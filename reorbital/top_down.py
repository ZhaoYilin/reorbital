...
class TOPDOWN:
    ...
    def divide(self, lap, abs_order):
        ...
        if self.type=='zero':
            S1 = [i for i, x in enumerate(fv) if x < 0]
            S2 = [i for i, x in enumerate(fv) if x >= 0]
        elif self.type=='median':
            m = median(fv)
            S1 = [i for i, x in enumerate(fv) if x < m]
            S2 = [i for i, x in enumerate(fv) if x >= m]
        elif self.type=='gap':
            g = np.argmax(np.diff(sorted(fv)))+1
            S1 = [i for i, x in enumerate(fv) if x in sorted(fv)[:g]]
            S2 = [i for i, x in enumerate(fv) if x in sorted(fv)[g:]]
